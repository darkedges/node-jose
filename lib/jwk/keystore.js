...
function importFrom(registry, input, pem) {
  // form can be one of:
  ...
  switch(form) {
    ...
    case "x509":
      ...
      if (result) {
        ...
        thumbprint256 = util.base64url.encode(Buffer.from(md256.digest().toHex(), "hex"));
        x5c = JWK.helpers.extractx5c(pem);
      }
      capture.type = "public";
      break;
  }
      ...
      if (Buffer.isBuffer(jwk) || typeof jwk === "string") {
        ...
        if ("json" === form) {
          ...
          try {
            ...
            jwk = importFrom(registry, factors, jwk);
            if (!jwk) {
